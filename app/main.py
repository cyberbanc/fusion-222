...
async def lifespan(app: FastAPI):
    ...
    db.init_db()
    state = db.get_state()
    print(f"UP-ONLY-STARTUP bank={state.get('bank')} cutoff={state.get('retro_cutoff_epoch')} retro_trades={state.get('retro_trades_count')} scope={state.get('retro_scope_version')} initialized={state.get('retro_initialized')} worker_enabled={SETTINGS.worker_enabled} tables={db.table_names()}", flush=True)
    try:
